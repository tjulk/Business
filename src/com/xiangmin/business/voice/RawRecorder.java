...
public class RawRecorder {
	private static final double LOG_OF_MAX_VOLUME = Math
	.log10((double) Short.MAX_VALUE);
	public enum State {
	...
	public List<Integer> totalScales = new ArrayList<Integer>();
		...
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			if (numberOfBytes == AudioRecord.ERROR_INVALID_OPERATION) {
				...
				try {
					...
					totalScales.add((int)(20 * Math.log10((double) mMaxAmplitude) / LOG_OF_MAX_VOLUME));
	...
	public List<Integer> getTotoalScales(){
		return totalScales;
	}
	...
	public void start() {
		if (mState == State.READY) {
			totalScales = new ArrayList<Integer>();
			mPayloadSize = 0;
	...
	public void stop() {
		if (mState == State.RECORDING) {
			Log.d(RawRecorder.class.getName(), "Stopping the recorder...");
